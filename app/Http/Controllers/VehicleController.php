...
class VehicleController extends Controller
{
	...
	{	
		...
		if($vehicle->user->featured)
			...
			$other_vehicle_text = 'Related Vehicles';
		SEO::setTitle($vehicle->year.' '.$vehicle->make->make_name.' '.$vehicle->model->model_name.' in Edmonton, Alberta $'.$vehicle->price);
        SEO::setDescription($vehicle->year.' '.$vehicle->make->make_name.' '.$vehicle->model->model_name.' '.($vehicle->trim?: "").' $'.$vehicle->price.' in Edmonton, Alberta for $'.$vehicle->price.'. Check out this '.($vehicle->ext_color->color?: "").' '.($vehicle->bodyStyleGroup->body_style_group_name?: "").' with '.($vehicle->odometer?: "").' kilometers on it for sale from '.($vehicle->user->name?: ""));
