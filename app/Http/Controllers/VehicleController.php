...
class VehicleController extends Controller
{
	...
	{	
		...
		$vehicle = Vehicle::with('user')->with('make')->with('model')->where('slug',$slug)->where('status_id', 1)->firstOrFail();
	...
	public function relatedVehicle(Request $request, $slug)
	{
		...
		// dd($paid_vehicles);
		$related = Vehicle::where(['make_id' => $vehicle->make_id, 'model_id'=>$vehicle->model_id, 'vehicles.status_id' => 1])
							->with('make')->with('model')->with('photos')
							->take(8)->get();
		$results = $paid_vehicles->merge($related);
		if($vehicle->user->featured)
		{
			$user_vehicles = Vehicle::where('user_id',$vehicle->user->id)->where(['vehicles.status_id' => 1])->with('make')->with('model')->with('photos')->take(10)->get();
			// dd($user_vehicles)
